...
interface ChatInterfaceProps {
  onSendMessage: (prompt: string, previousMessages?: OpenAIMessage[]) => Promise<string>;
}
...
export const ChatInterface = ({ onSendMessage }: ChatInterfaceProps) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const previousMessages: OpenAIMessage[] = messages.map(msg => ({
        role: msg.sender === "user" ? "user" : "assistant",
        content: msg.content
      }));

      const response = await onSendMessage(input, previousMessages);
